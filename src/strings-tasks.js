...
function removeFirstOccurrences(str, value) {
  ...
  if (index + value.length < str.length) {
    part2 = str.slice(index + value.length);
  }
...
function startsWith(str, substr) {
  if (typeof str === 'string' || str instanceof String) {
    if (typeof substr === 'string' || substr instanceof String) {
...
function formatTime(minutes, seconds) {
  ...
  let sec = '';
  if (minutes < 10) {
    min = min.concat('0', minutes.toString());
  } else {
    min = minutes.toString();
  }
  if (seconds < 10) {
    sec = sec.concat('0', seconds.toString());
  } else {
...
function countVowels(str) {
  let count = 0;
  if (typeof str === 'string' || str instanceof String) {
    if (str.match(/[aeioyu]/gi)) {
      count = str.match(/[aeioyu]/gi).length;
    }
  }
  return count;
}
...
function isPalindrome(str) {
  ...
  const reversed = str
    ...
    .replaceAll(re, '')
    .toLowerCase();
  return str.replaceAll(' ', '').toLowerCase().replaceAll(re, '') === reversed;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (
      (str.codePointAt(i) >= 65 && str.codePointAt(i) < 78) ||
      (str.codePointAt(i) >= 97 && str.codePointAt(i) < 110)
    ) {
      newStr[i] = String.fromCharCode(str.codePointAt(i) + 13);
    } else if (
      (str.codePointAt(i) >= 78 && str.codePointAt(i) <= 90) ||
      (str.codePointAt(i) >= 110 && str.codePointAt(i) <= 122)
    ) {
      newStr[i] = String.fromCharCode(str.codePointAt(i) - 13);
    } else {
      newStr[i] = str[i];
...
function getCardId(value) {
  ...
  let index1;
  const index2 = value.at(-1);
  if (value[0] === '1') {
    index1 = 9;
  } else {
    index1 = cards.indexOf(value.at(0));
  }
  let number = 0;
  if (index2 === '♣') {
    number = index1;
